...
with open('nlp python answer finals.txt', 'r', errors='ignore') as f:
    raw = f.read().lower()

with open('modules pythons.txt', 'r', errors='ignore') as m:
    rawone = m.read().lower()
...
Introduce_Ans = ["My name is PyBot.","My name is PyBot you can called me pi.","I'm PyBot :) ","My name is PyBot. and my nickname is pi and i am happy to solve your queries :) "]
...
def chat():
    user_response = request.json['message'].lower()
    keyword = " module "
    keywordone = " module"
    ...
    if user_response != 'bye':
        ...
        elif basicM(user_response):
            return jsonify({'response': basicM(user_response)})
        elif any(kw in user_response for kw in [keyword, keywordone, keywordsecond]):
            return jsonify({'response': response(user_response, sent_tokensone)})
        elif greeting(user_response):
            return jsonify({'response': greeting(user_response)})
        elif "your name" in user_response:
            return jsonify({'response': IntroduceMe(user_response)})
        elif basic(user_response):
            return jsonify({'response': basic(user_response)})
        else:
            return jsonify({'response': response(user_response, sent_tokens)})
    else:
